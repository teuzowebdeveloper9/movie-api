...
| `mongodb.yaml` | StatefulSet com PVC (1Gi) + headless Service |
| `rabbitmq.yaml` | Deployment + Service + Secret de credenciais |
...
- **Configuração:** ConfigMaps para valores não sensíveis e Secret para as credenciais do RabbitMQ;
...
- O Secret do RabbitMQ tem valor de exemplo versionado apenas para o desafio — em um cluster real viria de um cofre (External Secrets, Sealed Secrets, Vault);
